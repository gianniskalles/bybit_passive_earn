...
| Product open | `status == "Available"` and `remaining_capacity >= amount` |
| Liquid | `redemption_eta_hours <= MAX_REDEMPTION_ETA_HOURS` |
| Size sane | `amount >= min_stake_amount` and `amount >= MIN_MOVE_USD` |
...
remaining_capacity, MAX_PER_PRODUCT_USD)`.
  ...
  "decisions": [
    {
      ...
      "product_id": "430",
      "amount": 5,
      "reason": "apr_ma_24h 0.0198 >= ENTRY_APR 0.018; marginal 0.021 >= ENTRY_APR 0.018; eta 0.0 <= 2; amount 5 >= min_stake 1",
